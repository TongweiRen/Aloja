...
def filter_public_structs(current_structs):
    ...
    'in_addr', 'sockaddr_in6', 'in6_addr', 'sockaddr_storage', 'std::_Rb_tree_iterator', 'in6_addr', 'ipv6_mreq']
...
def Func0(datas, funcname, cur_pos):
    ...
    for i in range(len(datas)):
        if "call " in datas[i] and f" @{funcname}(" in datas[i] and not datas[i].startswith("  tail"):
            ...
            try:
                ...
                if ',' in numb_1:
                    numb_1 = numb_1.split(",")[0]
            except:
                numb_1 = ''
            # console.print(datas[i])
            # console.print(datas[i].split(" = ")[0].split("%"))
    ...
    for i in range(len(target_function_contents)):
        if f"%{numb_1}" in target_function_contents[i] and not target_function_contents[i].startswith("  call void @llvm.dbg.declare"):
            ...
            if m.count("(", 0, len(m)) == 0:
                if "%struct" in m:
                    TP_struct = m.split("struct.")[1].split("*")[0]
                    break
                else:
                    numb_2 = m.split("%")[1].split(",")[0]
                    break
            p = re.compile(r"[(](.*?)[)]", re.S)
...
def reverseSocketFunc(IR_file):
    ...
    if flag_send:
        ...
        if a not in TP_structs and a != "Nothing":
            TP_structs.append(a)
    if flag_recv:
        a = Func3(datas, "recv", 0).split(",")[0]
        a = a.strip("]")
        a = a.strip("\"")
        a = a.strip("*")
        if a not in TP_structs and a != "Nothing":
            TP_structs.append(a)
        a = Func3(datas, "recv", 1).split(",")[0]
        a = a.strip("]")
        a = a.strip("\"")
        a = a.strip("*")
        if a not in TP_structs and a != "Nothing":
...
def reverseMemcpyFunc(IR_file, TP_functions):
    ...
    while flag_memcpy:
        for i in range(len(datas)):
            if "call " in datas[i] and " @llvm.memcpy.p0i8.p0i8.i64(" in datas[i]:
...
def controlTPFilters(IR_file, TP_functions):
    ...
    TP_Structs = filter_public_structs(TP_Structs)
