...
def filter_public_structs(current_structs):
    ...
    'in_addr', 'sockaddr_in6', 'in6_addr', 'sockaddr_storage', 'std::_Rb_tree_iterator', '__pthread_internal_list', '__pthread_mutex_s', '__va_list_tag']

    len_1 = len(current_structs)

    dupli = deepcopy(current_structs)

    for item in public_structs:
        for item_2 in dupli:
            if item in item_2 or item_2 in item:
                # console.print(item)
                # console.print(item_2)
                if item_2 in current_structs:
                    current_structs.remove(item_2)
                    break
        # if item in current_structs:
        #     current_structs.remove(item)
    # len_2 = len(current_structs)
    # console.print(current_structs)
    dupli2 = deepcopy(current_structs)
    for item in public_structs:
        for item_2 in dupli2:
            if item in item_2 or item_2 in item:
                # console.print(item)
                # console.print(item_2)
                if item_2 in current_structs:
...
def Func1(target_function_contents, pre_numb, new_pos):
            ...
            if "struct." not in target_function_contents[i]:
                ...
                if len(numbs) >= 2:
                    if "getelementptr inbounds %union." in target_function_contents[i]:
                        ...
                        if 'getelementptr inbounds' in target_function_contents[i]:
                            new_numb = numbs[2]
                        else:
                            new_numb = numbs[1]
                        if check_numb == new_numb:
                ...
                if "struct." in target_function_contents[i-1]:
                    TP_struct = target_function_contents[i-1].split("struct.")[1].split("*")[0]
                    # console.print(f"[red]_________[/red] {TP_struct}")
                else:
            ...
            for i_a in range(len(a)):
                if f"%{pre_numb}" in a[i_a]:
                    if "%struct" in a[i_a]:
                        ...
                        new_pos = str(i_a)
                        # console.print(f"in Func1_6 [red][+][/red] {new_pos}")
...
def Func2(datas, funcname, pos):
    ...
    Flag = True
    while(Flag):
        # console.print(f"in Func2_3 [red][+][/red] {target_function_contents[0]}")
        numb, new_funcname, new_pos, TP_struct = Func1(target_function_contents, numb, new_pos)
...
def reverseSocketFunc(IR_file):
    ...
    if flag_write:
        ...
        if a not in TP_structs and a != "Nothing":
            TP_structs.append(a)
            # console.print(f"[bold magenta][+][/bold magenta] After analyze send function, TP_struct is: {a}")
