...
def parse_callgraph(callgraph_data_server, callgraph_data_client, server_map, client_map, struct, all_contents):
    ...
    ''' First way to minimize function list. '''
...
def main(server_IR, client_IR):
    ...
    structs = PaCo_data['TP_structs']
    # console.print(structs)
    

    all_contents = {}
    for item in structs:
        functions_for_one_struct_server = PaCo_data[item]['functions_for_one_struct_server']
        functions_for_one_struct_client = PaCo_data[item]['functions_for_one_struct_client']
        test = GetStructsAndFields(server_IR)
        if '.' in item:
            struct_name = item.split('.')[0]
        else:
            struct_name = item
        ...
        try:
            ...
            all_contents = parse_callgraph(callgraph_data_server, callgraph_data_client, server_map, client_map, item, all_contents)
        except:
            continue
    
    results = json.dumps(all_contents, sort_keys=False, indent=4, separators=(',', ': '))
    with open("temp_outputs/callgraph_contents.json", 'w') as f:
        f.writelines(results)
