...
class InsertReadWrapperCall:
    ...
    def insertReadWrapperCall(self):
        ...
        # console.print(temp_GEPs)
        # for i in range(0, len(target_function_contents)):
        #     if target_field_symbol_1 in target_function_contents[i]:
        #         console.print(f"{target_function_contents[i]}")
        ...
        for i in range(len(target_function_contents)):
            ...
            # if temP1 in target_function_contents[i] and 'store' in target_function_contents[i]:
                inject_point = i + 1
        ...
        # console.print(added_instruction_number-len(temp_GEPs)-1)

        
        for item in GEPs:
            added_instructions.append(item)
        current_numb = added_instruction_number + len(added_instructions) - 1
        instruction_0 = f"  %{current_numb+1} = load i32, i32* %{current_numb}, align 4\n"
        instruction_1 = f"  %{current_numb+2} = call i32 @Aloja_Wrapper_Function(i32 %{current_numb+1})\n"
        ...
        instruction_store = f"  store i32 %{current_numb+2}, i32* %{current_numb_2}, align 4\n"
...
if __name__ == '__main__':
    ...
    # test = InsertReadWrapperCall("bitcode/wakaama/lwm2mserver_mutated.ll", "bitcode/wakaama/lwm2mserver_mutated.ll", "coap_packet_t", "payload_len", 37 , "coap_serialize_get_size")
