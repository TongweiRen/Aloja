...
main(int argc, char** argv) {
  ...
  // Test later to see if we need to actually preserve `main` or
  // if we can set a different entry and skip this check.
  auto* mainFunction = module->getFunction("main");
  if (!mainFunction) {
    //llvm::report_fatal_error("Unable to find main function.");
    llvm::outs() << "Unable to find `main` function. Trying `packet__write`\n";
    mainFunction = module->getFunction("packet__write");
  }
